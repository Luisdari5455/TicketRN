...
import { BackHandler, Platform, View, AppState, AppStateStatus, Modal, Text, TextInput, Pressable } from 'react-native';
    ...
    <SafeAreaProvider>
      ...
      {!isUnlocked ? (
        <View style={{ flex: 1, backgroundColor:'#0f172a', alignItems:'center', justifyContent:'center', padding:24 }}>
          ...
          <TextInput
            value={entryPin}
            onChangeText={(t)=>setEntryPin(t.replace(/[^0-9]/g,'').slice(0,6))}
            placeholder="PIN"
            placeholderTextColor="#94a3b8"
            keyboardType="number-pad"
            secureTextEntry
            autoFocus
            style={{ backgroundColor:'#fff', borderRadius:10, paddingVertical:12, paddingHorizontal:16, fontSize:20, width:280, textAlign:'center' }}
            returnKeyType="done"
        ...
        <>
          ...
          <Pressable onPress={handleSecretTap} style={{ position:'absolute', top:0, left:0, width:100, height:100 }} />
        </>
      )}

      {/* Modal de PIN de salida */}
      <Modal transparent visible={unlockVisible} animationType="fade" onRequestClose={() => setUnlockVisible(false)}>
        <Pressable style={{ flex:1, backgroundColor:'rgba(0,0,0,0.6)', alignItems:'center', justifyContent:'center', padding:24 }}
          ...
          <Pressable onPress={()=>{}} style={{ width:360, maxWidth:'95%', backgroundColor:'#0f172a', borderRadius:16, padding:20 }}>
            <Text style={{ color:'#fff', fontSize:18, fontWeight:'700', marginBottom:12 }}>Salir (PIN requerido)</Text>
            <TextInput
              value={exitPin}
              ...
              secureTextEntry
              autoFocus
              style={{ backgroundColor:'#fff', borderRadius:8, paddingVertical:12, paddingHorizontal:14, fontSize:18, marginBottom:16, textAlign:'center' }}
              returnKeyType="done"
            ...
            <View style={{ flexDirection:'row', justifyContent:'flex-end', gap:12 }}>
              ...
              <Pressable onPress={tryExitUnlock} style={{ backgroundColor:'#1e40af', paddingVertical:10, paddingHorizontal:16, borderRadius:8 }}>
